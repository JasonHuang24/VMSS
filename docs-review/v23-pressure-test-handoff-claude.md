...
**Run 3 (low-effort experiment):** Claude, **lowest-effort configuration available** (e.g. fast mode / smallest thinking budget the UI offers — record which). Prompt below stays byte-identical; only the effort knob moves. Registered predictions from the run-2 findings: attribution collapse deepens and first fabricated instruments/numbers may appear; possible superseded-pass citations (zero so far in all four runs); shallower envelope sweeps; shorter rulings. Corpus note: attached files include enacted LP-077–079 beyond the prompt's stated LP-076 range — finding that territory covered is valid, and whether the model *notices* the range extension unprompted is itself a data point (GPT-5-medium did; Opus-medium did not remark on it).
