...
2. **S17 friction — initially CONFIRMED by the verification sweep, then RESOLVED as a false seam by Jason's ruling (2026-07-22): no canon change.** The sweep correctly found no *birth-attribution* clause — but the question dissolves because the escalation curve is not a birth-event tax. Article XXVII's own text (charter.html:419) defines it as a **household-composition instrument** ("every fiscal instrument applicable to the household is raised in concert… compounding per child beyond the threshold"): the schedule keys to the household's child count in a curve layer, continuously. A fifth child born in -2 and kept there never enters the count (territorial, no double standard between VPR and elective residents); a fifth child brought into the Main household raises the count to five and the wall fires on composition, with no locational clause needed. The child's own Article VIII route lands in Main *autoparenting facilities* (charter.html:421/424), not the parental household, so the walk-up channel doesn't smuggle the count either. Lower-layer scale abuse is separately closed by the structural treasury-levy cascade at charter.html:424–425, which explicitly covers -2 — the paragraph the run-2 model (and this review's first pass) stopped short of. Same logic resolves the LP-077 analogue: enrollment "counts on the LP-064 escalation schedule" against the *originator's* household wherever instantiation occurs, and a genuine VPR -2 originator sits outside the curve but inside the levy cascade. **Queue candidate 10 withdrawn.**
...
- ~~New queue candidate (item 10)~~ **WITHDRAWN** — S17 resolved as a false seam per Jason's ruling; the household-composition reading of XXVII plus the -2 treasury-levy cascade close both ends with existing text (see adversarial finding 2).
- **S20 pre-queue check DONE:** Whitepaper §17.1.5 carries the Duress Signaling envelope (implant involuntary-state telemetry flags coercion to the federal floor, whitepaper.html:1065) — the sensor exists in canon; the only unstated piece (quarantining a duress-flagged refusal directive) folds into existing queue item 5.
