...
Setup: verify `git remote -v` → JasonHuang24/VMSS. Create branch feat/charter-loadbearing-audit from origin/main (post latent-merge; if the latent branch is unmerged, branch from origin/feat/vmss-laws-latent-corpus instead). Read fully, in order: docs-review/vmss-laws-loadbearing-handoff.md (the spec for THIS run — §3–§6 define the deliverable), docs-review/vmss-laws-architecture.md (§5 is the validation prior), docs-review/vmss-laws-latent-inventory.md (the B8 parent-authority and charter-touchpoints fields seed your E1 edges), docs-review/vmss-laws-latent-dispositions.md and vmss-laws-latent-final-review.md (ratified judgments you inherit), docs-review/vmss-laws-opus-prompts.md Prompt 0 (hard rules).
...
Gate: the register is the must-complete deliverable — if the time box expires with graph + register committed, the run succeeded. check-canon must stay green throughout (you touch only docs artifacts). Push the branch (git push -u origin feat/charter-loadbearing-audit) and STOP: Jason personally ratifies the register before any Run L2 enactment exists. No PR, no merge, never main.
```
