...
const PROBES = [
  ...
  { family: 'register: house style, ls- forms (REGRESSION)', expect: 'house style: register entries carry no working-document apparatus',
    label: 'a register entry grows an ls-code block — statute typography the guard could not see, alongside ls-table and ls-ruling',
    edits: [{ file: 'law-polling.html', find: '</article>', replace: '<div class="ls-code">x</div></article>' }] },
  { family: 'register: vote cells with class lists (REGRESSION)', expect: 'vote tables match declared outcomes',
    label: 'a failing gate restyled to class="fail is-narrow" stopped counting as a failure',
    edits: [
      { file: 'law-polling.html', find: 'class="status-badge status-failed"', replace: 'class="status-badge status-enacted"' },
      { file: 'law-polling.html', find: '<td class="fail"', replace: '<td class="fail is-narrow"', all: true }] },
  { family: 'register: stat cards agree (REGRESSION)', expect: 'stat card: Entries',
    label: 'a second Entries stat card states a stale total beside the correct one — existence-testing passed, agreement does not',
    edits: [{ file: 'law-polling.html', find: '<h2 class="law-section-header" id="regulatory-petitions">', replace: '<p>>95</p>\n<p class="text-xs text-[var(--text-muted)]">Entries<</p><h2 class="law-section-header" id="regulatory-petitions">' }] },
  ...
  { family: 'consolidation purity: dash forms (REGRESSION)', expect: 'consolidation purity',
    label: 'a relocated magnitude returns to Charter tier written with an EN DASH — the form charter.html actually uses for numeric ranges, and the form that used to slip past',
    edits: [{ file: 'charter.html', find: '</body>', replace: '<p>Downward conversion carries a 90–99% forfeiture that prevents arbitrage.</p></body>' }] },
  ...
  { family: 'process record: R22 word boundary (REGRESSION)', expect: 'R22 is registered',
    label: 'R22 renumbered to R220 — a bare substring test could not tell the two apart',
    edits: [{ file: 'pending-ratification.html', find: 'R22', replace: 'R220', all: true }] },
  { family: 'code (e): sitemap <loc> (REGRESSION)', expect: 'code integrity (e)',
    label: 'the Code drops out of the sitemap but the string survives in a comment — a substring test called that listed',
    edits: [{ file: 'sitemap.xml', regex: true, find: '<loc>([^<]*)/laws\\.html</loc>', replace: '<!-- was $1/laws.html -->' }] },
  { family: 'code (d1): heading census (REGRESSION)', expect: 'code integrity (d1)',
    label: 'a new non-article <h2 id> lands on the Charter with no Tier 1 row — the articles+2 literal could not see it',
    edits: [{ file: 'charter.html', find: '</body>', replace: '<h2 id="closing-note">Closing Note</h2></body>' }] },
  ...
  { family: 'nested cert suite: count floor (REGRESSION)', expect: 'hostile certification mutation suite',
    label: 'the nested suite reports the right shape with an empty mutation table — "0 rejected, 0 accepted" proved nothing and passed',
    edits: [{ file: 'tools/test-path2-certification-mutations.mjs', find: '2272 hostile mutations rejected', replace: '0 hostile mutations rejected' }] },
