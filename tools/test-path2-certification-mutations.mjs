...
test('complete committed authority record verifies the lawful Finding III failure', () => { const result = evaluateAuthorityRecord(data, { root: ROOT }); return result.valid === true && result.lawfulFailure === true && result.certified === false; });
...
test('committed Findings reproduce PASS/PASS/FAIL/PASS', () => { const result = evaluatePath2Findings(data.path2Findings, knownSources.map((source) => source.id)); return result.I.pass && result.II.pass && !result.III.pass && result.IV.pass; });
test('Finding I least-favorable year controls', () => { const value = clone(data.path2Findings); value.I.members[1].horizon[29].treatmentCoverageLowerBound = 0.999; value.I.disposition = 'FAIL'; value.I.votes.forEach((vote) => { vote.vote = 'REFUSE'; }); return evaluatePath2Findings(value).I.pass === false; });
...
test('NaN Finding II bound is rejected', () => { const value = clone(data.path2Findings); value.II.members[0].horizon[0].attributedTreatmentDividendLowerBound = NaN; return evaluatePath2Findings(value).II.detail.includes('attributed dividend bound'); });
test('nonnumeric Finding II bound is rejected', () => { const value = clone(data.path2Findings); value.II.members[0].horizon[0].attributedTreatmentDividendLowerBound = 'not-a-number'; return evaluatePath2Findings(value).II.detail.includes('attributed dividend bound'); });
...
test('interval exceeding precision floor requires a void-for-indecision disposition', () => { const value = clone(data.path2Findings); const check = value.III.members[0].interval.precisionChecks[0]; check.width = check.ceiling + 0.01; check.passed = false; value.III.members[0].interval.precisionSatisfied = false; return evaluatePath2Findings(value).III.errors.some((error) => error.includes('disposition or Commission votes')); });
test('exact-threshold boundary fails', () => { const value = clone(data.path2Findings); for (const member of value.I.members) for (const row of member.horizon) row.treatmentCoverageLowerBound = 1.01; value.I.members[1].horizon[0].treatmentCoverageLowerBound = 1; value.I.leastFavorableMember = value.I.members[1].id; value.I.disposition = 'FAIL'; value.I.votes.forEach((vote) => { vote.vote = 'REFUSE'; }); return evaluatePath2Findings(value).I.pass === false; });
...
test('missing §4.3 row-level validation record is rejected', () => { const value = clone(executionOutput); value.validationRecords.pop(); return lockedErrors({ executionOutput: value }).some((error) => error.includes('§4.3 outcome-specific')); });
...
test('authored horizon bound without recomputation is rejected', () => { const value = clone(executionOutput); value.findings.I.members[0].horizon[0].treatmentCoverageLowerBound += 0.01; return lockedErrors({ executionOutput: value }).some((error) => error.includes('independent recomputation')); });
test('arbitrary deposited precision ceiling is rejected', () => { const value = clone(data); value.path2Findings.I.members[0].interval.precisionChecks[0].ceiling = 999; return evaluateAuthorityRecord(value, { root: ROOT }).valid === false; });
